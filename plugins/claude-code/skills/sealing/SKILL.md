...
The CLI is `npx -y taleseal@latest`; with no `--transcript` flag it picks the newest Claude
Code transcript for the current directory, which in a live session is the session itself.
...
```bash
npx -y taleseal@latest seal --skeleton /tmp/taleseal-skeleton.json
```
...
Ask with AskUserQuestion: publish as previewed, adjust the story first, or cancel. If the
