...
The CLI is `npx -y taleseal@latest` (plain Node, no install). `--cursor` picks the newest
Cursor transcript for this project, which in a live session is usually the session itself —
if several Cursor windows are open, pin an explicit path with `--transcript <path>` (in a
hook, `--transcript "$CURSOR_TRANSCRIPT_PATH"`).
...
```bash
npx -y taleseal@latest seal --cursor --skeleton /tmp/taleseal-skeleton.json
```
